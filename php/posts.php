    ...
    <div class="post-list-outline">
        ...
        if($row['status'] === 0) { ?>
            ...
            if($type !== 1) { ?>
                ...
                <?php if(empty($_SESSION['username'])) { ?>
                    <div class="guest-message">
                        <p>You must sign in to post a comment.<br><br>Sign in with a Galaxy Plaza account to connect to users around the world by writing posts and comments and by giving Yeahs to other people's posts. You can sign up for a Galaxy Plaza account <a href="/signup">here</a>.</p>
                        <a href="/login" class="arrow-button"><span>Log In</span></a>
                        <a href="/signup" class="arrow-button"><span>Sign Up</span></a>
                        <a href="/info/rules" class="arrow-button"><span>Galaxy Plaza Rules</span></a>
                    </div>
            ...
            <div id="report-violation-page" class="dialog none" data-modal-types="report report-violation" data-is-template="1">
                <div class="dialog-inner">
                    <div class="window">
                        <h1 class="window-title">Report Violation to Galaxy Plaza Administrators</h1>
                        <div class="window-body">
                            <p class="description">You are about to report a <?=$object?> with content which violates the Miiverse Code of Conduct. This report will be sent to the Galaxy Plaza administrators and not to the creator of the post.</p>
                            <form method="post" action="/<?=htmlspecialchars($_GET['type'] . '/' . $_GET['id'])?>/violations">
                                ...
                                <select name="type" class="can-report-spoiler<?=(isset($row['community']) && $row['community'] === null) || (isset($oprow['community']) && $oprow['community'] === null) ? ' none' : ''?>">
                                    ...
                                    <option value="1" data-body-required="1"<?=(isset($row['community']) && $row['community'] === null) || (isset($oprow['community']) && $oprow['community'] === null) ? ' selected' : ''?>>Galaxy Plaza Staff</option>
                                </select>
