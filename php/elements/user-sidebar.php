    ...
    <div class="sidebar-container">
        ...
        <div id="sidebar-profile-body"<?=$row['favorite_post'] !== null ? ' class="with-profile-post-image"' : ''?>>
            ...
            if(!$stmt->error) {
                ...
                if($brow['COUNT(*)'] > 0) {
                    $banned = true;
                    $status = '<span style="color:red;" class="user-status">Banned</span>';
                } else {
                    $banned = false;
                    if(time() - 30 < strtotime($row['last_seen'])) {
                        $status = '<span style="color:lime;" class="user-status">Online</span>';
                    } else {
                        $status = '<span style="color:gray;" class="user-status">Offline</span>';
                    }
                }
				print($status);
                        ...
                        <div class="window">
                            ...
                            <div class="window-body">
                                <form method="post" action="/users/<?=htmlspecialchars(urlencode($row['username']))?>/violators">
                                    ...
                                    <p class="description">You are about to report a user for violating the <?php print(SITE_NAME); ?> Rules. This report will be sent to the Galaxy Plaza administrators and not to the user you are reporting or any community staff.</p>
